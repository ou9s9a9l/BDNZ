		...
		for ( b=0;b<7;b++)
			for (c=0;c<150;c++)
				if (x_middle[a][b]==rdat[c]&&x_middle[a][b]!=0)//reserve条件成立
					{
					if(b==0&&find(rdata3,x_left[a][0])&&find(rdata3,x_left[a][1])&&find(rdata3,x_left[a][2])&&find(rdata3,x_left[a][3])&&find(rdata3,x_left[a][4])&&find(rdata3,x_left[a][5])&&find(rdata3,x_left[a][6]))//reserver左边判断
					{DoWithArray[DoWithCount++]=x_middle[a][b];
					DoWithArray[DoWithCount++]=a;}
					else 
					if(b==1&&find(rdata3,x_right[a][0])&&find(rdata3,x_right[a][1])&&find(rdata3,x_right[a][2])&&find(rdata3,x_right[a][3])&&find(rdata3,x_right[a][4])&&find(rdata3,x_right[a][5]))//reserver又边判断
					{DoWithArray[DoWithCount++]=x_middle[a][b];
					DoWithArray[DoWithCount++]=a;}
...
while(o_count<CON2_LENGTH)
	{
		...
		//4个方向的轨道成功
			...
			d=0;
			tempa=0;
			if(factor[o_count].what)// 边上白色有了就不成功
				{d=1;}//轨道边
		/*	if (tempb==5&&0==d)//5的时候用来写两边的两个
			...
			/*if (bcastflag==1&&bcastnum==a)
			{
				...
				if(!find(rdata3,signal[1])||o_count!=1||(DoWithS[a].howmany==5&&!find(rdata3,signal[3])))//非红灯
					{
